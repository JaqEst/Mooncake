...
from mooncake.integration.paddle import Buffer
from ep_test_utils import init_dist, bench, bench_kineto, calc_diff, hash_tensor, per_token_cast_back


def test_main(num_tokens: int, hidden: int, num_experts: int, num_topk: int,
              rank: int, num_ranks: int, group: Group, buffer: Buffer, seed: int = 0):
    paddle.manual_seed(seed + rank)
    ...
    x = paddle.ones((num_tokens, hidden), dtype=paddle.bfloat16) * (rank - rank_offset)
    x[:, -128:] = paddle.arange(num_tokens).to(paddle.bfloat16).view(-1, 1)
    scores = paddle.randn((num_tokens, num_experts), dtype=paddle.float32).abs() + 1
    topk_idx = paddle.topk(scores, num_topk, dim=-1, largest=True, sorted=True)[1]
    topk_weights = paddle.randn((num_tokens, num_topk), dtype=paddle.float32).abs()
    ...
    active_ranks = paddle.ones((num_tokens, ), dtype=paddle.int32)
    for return_recv_hook in (False, True):
        for dispatch_use_fp8 in (False, True):
            ...
            all_topk_idx = paddle.empty((num_ranks, num_tokens, num_topk), dtype=topk_idx.dtype)
            dist.stream.all_gather(all_topk_idx, topk_idx, group=group)
            for i in range(num_local_experts if do_check else 0):
                ...
                int_mask = paddle.to_tensor((1 << 32) - 1, dtype=recv_layout_range.dtype, place=recv_layout_range.place)
                ...
                recv_x_amin = recv_x[:, :-128].cast(paddle.float32).amin(dim=-1)
                recv_src_info = recv_src_info[:num_valid_tokens]
                assert paddle.compat.equal(recv_x_amin, recv_x[:, :-128].cast(paddle.float32).amax(dim=-1))
                assert (recv_x[:, -128:] - paddle.cast(recv_src_info.view(-1, 1) % num_tokens, paddle.bfloat16)).sum().item() == 0
            ...
            for zero_copy in (False, True):
                ...
                out = paddle.empty((num_tokens, hidden), dtype=paddle.bfloat16)
                ...
                if do_check:
                    ...
                    assert paddle.isnan(combined_x).sum().item() == 0
                    assert diff < 1e-5, f'Error: {diff=}, {zero_copy=}'
                    hash_value ^= hash_tensor(combined_x)

    def create_test_cast_with_outliers(num_outliers):
        tmp = paddle.randn((num_tokens, hidden), dtype=paddle.bfloat16)
    ...
    def large_gemm_with_hook(hook):
        mat_0 = paddle.randn((8192, 8192), dtype=paddle.float)
        mat_1 = paddle.randn((8192, 8192), dtype=paddle.float)
        mat_0 @ mat_1
        hook()
