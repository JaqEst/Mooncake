...
def make_tokens(num_tokens: int, hidden: int, rank: int) -> paddle.Tensor:
    x = paddle.full((num_tokens, hidden), float(rank),
                   dtype=paddle.bfloat16, )
    x[:, -128:] = paddle.arange(num_tokens, ).to(
        paddle.bfloat16).view(-1, 1)
    return x
...
def make_routing(num_tokens: int, num_logical_experts: int,
    ...
    paddle.seed(seed)
    scores    = paddle.rand([num_tokens, num_logical_experts])
    topk_idx  = scores.topk(num_topk, dim=-1).indices.to(paddle.int64)
    topk_weights = (paddle.rand([num_tokens, num_topk],
                                dtype=paddle.float32).abs() + 0.1)
    return topk_idx, topk_weights


def make_active_ranks(n: int) -> paddle.Tensor:
    return paddle.ones(n, dtype=paddle.int32, )
...
class ExpertRemapper:
    def __init__(self, phy2log: List[int], num_logical_experts: int):
        ...
        self._first   = paddle.tensor([s[0] for s in log2phy],
                                     dtype=paddle.int64, )
        self._padded  = paddle.full((num_logical_experts, max_rep), -1,
                                   dtype=paddle.int64, )
        self._counts  = paddle.zeros(num_logical_experts,
                                    dtype=paddle.int64, )
    ...
    def remap(self, topk_logical: paddle.Tensor) -> paddle.Tensor:
        mask = topk_logical >= 0
...
def print_routing_stats(rank: int, role: str, topk_idx: paddle.Tensor,
...
def verify_correctness(x, topk_idx, topk_weights, combined_x,
    ...
    nan_cnt  = int(paddle.isnan(combined_x).sum())
...
def worker_t_kill(rank: int, barrier_dir: str,
    ...
    x = (make_tokens(NUM_TOKENS, HIDDEN, rank)
         if role == "attention"
         else paddle.empty(0, HIDDEN, dtype=paddle.bfloat16, ))
...
def worker_t_recovery_rank3_orig(rank: int, barrier_dir: str, port: int):
    ...
    x = paddle.empty(0, HIDDEN, dtype=paddle.bfloat16, )
    tidx_log, tw = make_routing(NUM_TOKENS, NUM_LOGICAL, NUM_TOPK,
                                seed=20 + rank)
    tidx_phy = remapper.remap(tidx_log)
...
def worker_t_recovery_rank3_new(rank: int, barrier_dir: str,
    ...
    dist.init_process_group(
        ...
        world_size=4,
        pg_options=pg.MooncakeBackendOptions(
            paddle.ones(4, dtype=paddle.int32, ),
